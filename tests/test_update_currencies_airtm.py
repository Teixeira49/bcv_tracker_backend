...
def test_update_currencies_solo_airtm_persiste(monkeypatch):
    airtm_rows = [
        venezuela_controller.dollar_service.createCurrency("USD", "Dolar-Buy", 1.0, c.AIRTM_NAME),
        venezuela_controller.dollar_service.createCurrency("USD", "Dolar-Sell", 2.0, c.AIRTM_NAME),
    ]
    monkeypatch.setattr(
        venezuela_controller.dollar_service, "get_raw_airtm_currencies",
