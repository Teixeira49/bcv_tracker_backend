...
2. **`.env.example`**: agrega/renombra/elimina la clave. Sus claves deben **coincidir exactamente** con las variables requeridas de `_REQUIRED_ENV` (ni de más ni de menos). Sin valores sensibles: usa el proveedor público o un placeholder (ej. `postgresql://usuario:password@localhost:5432/nombre_db`).
3. **README** (sección "Configurar variables de entorno"): documenta la variable con una línea/comentario de qué es.
